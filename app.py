...
def index():
    ...
    cur = db.cursor()
    userID = user['userID']
    activities = cur.execute("""SELECT * FROM Activities RIGHT JOIN ActivitiesUsers ON Activities.activityID = ActivitiesUsers.activityID WHERE Activities.UserID = (?) """, [userID]).fetchall()
